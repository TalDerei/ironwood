...
This module models that apparatus and proves it peels off: under the augmented binding (the SRS generators
together with `U`, `W` are linearly independent — the discrete-log assumption extended to the two extra
fixed generators), a deployed group equation `⟨a, g⟩ + α•U + β•W = ⟨a', g⟩ + α'•U + β'•W` splits
coordinate-wise into `a = a'`, `α = α'`, `β = β'`. So the deployed verifier's combined check separates into
the clean `IpaAcceptV` checks (the `g`-side commitment and the `U`-side value) plus a discarded `W`-side
blinding identity — closing the `U`/`W`/`S` structure down to `ipa_soundV`. The residual floor is the
standard one: the special-soundness rewinding (the transcript tree) and the discrete-log/AGM binding.
-/
...
extra fixed generators `U`, `W` that the deployed IPA verifier uses — the generators halo2 treats as
...
`assemble.eval = 0` with no posited tree (the opening, AGM route). With the prover's group elements given
via their AGM representations over `(g, U, W)` — the multiopen commitment `commitGen g aP + pw•W`, the
blinding poly `commitGen g aSxi + swxi•W` (the `ξ`-scaled `S`), the round total
`commitGen g aRounds + uRounds•U + wRounds•W`, the `[-v]g₀` term `commitGen g vVec`, the `[-c·b·z]U` and
`[-f]W` constants, and the folded generator `commitGen g svTerm` (`= [-c]·G'₀`, via
`foldAll = commitGen g (sFun u 1)`) — the augmented binding reads `assemble.eval = 0` off as three
independent identities: the `g`-relation (the opening constraint), the `U`-relation (the
value/inner-product binding), and the `W`-relation (blinding). This is the MSM↔structured-checks
connection, as a proof from `assemble.eval = 0` rather than a posited bridge. Floor: the AGM
representations and the augmented binding. -/
...
/-- The value `⟨aP, b⟩ = v` follows from the split relations (the opening's value extraction). From the
`g`-relation (`deployed_flat_split`'s first conjunct), taken in inner product with `b`, plus the
`U`-relation and the honest/Schwartz–Zippel structure: the blinding poly vanishes at the point
(`⟨aSxi, b⟩ = 0` — `S` has a root at `x`, the `ξ`-challenge's job), the round `U`-coefficient is
`z·⟨aRounds, b⟩` (the `z`-challenge ensures the prover cannot interfere with `U`), and the constants are
`⟨vVec,b⟩ = -v` (the `[-v]g₀` term, `b₀ = 1`) and `⟨svTerm,b⟩ = -cb` (`cb = c·compute_b`, the folded
generator's value). The `U`-relation forces `⟨aRounds,b⟩ = cb` (`z ≠ 0`), and the `g`-relation then
collapses to `⟨aP,b⟩ = v`. So the extracted witness opens to the claimed value. Floor: the Schwartz–Zippel
challenge exclusions (`z`, `ξ`) named as `huRounds`/`hSxi`. -/
...
blinding identity `blind = f`. The `z ≠ 0` cancellation is halo2's "challenge that ensures the prover did
not interfere with the `U` term". -/
...
only inputs are the floor: the augmented binding (DLR/AGM, `hbind`) and the transcript tree itself (the
