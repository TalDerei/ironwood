...
per-round folding rests on the linearity proved here. Curve-group binding / the AGM stays an explicit
assumption, per project scope.
-/
...
generators are `F`-linearly independent). On the Vesta curve this is the discrete-log /
...
independent. On Vesta this is the discrete-log / AGM assumption. -/
